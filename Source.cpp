...
int main() {
	...
	R3 n = _M_.a * h + _M_.b * k + _M_.c * l;    //вектор нормали к плоскости n(hkl)
	double cos_θ = λ * n.length()/2;            //дифракция


	//угол горизонтального поворота в сферических координатах для вектора n
	double alpha = (n.y >= 0) ?
		phi({ n.x, n.y,0 }, { 1, 0, 0 }) :
		-phi({ n.x, n.y,0 }, { 1, 0, 0 });

	//угол вертикального поворота в сферических координатах для вектора n
